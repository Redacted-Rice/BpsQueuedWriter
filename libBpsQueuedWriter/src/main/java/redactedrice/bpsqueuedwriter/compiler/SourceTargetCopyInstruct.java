...
public class SourceTargetCopyInstruct extends FixedLengthInstruction {
    ...
    public static SourceTargetCopyInstruct create(String key, String[] args) {
        ...
        try {
            return new SourceTargetCopyInstruct(Integer.parseInt(args[0]),
                    Integer.parseInt(args[1]), type);
        } catch (IllegalArgumentException iae) {
            throw new IllegalArgumentException(supportedArgs + "given: " + Arrays.toString(args)
                    + " and encountered error: " + iae.getMessage());
        }
    }
    ...
    public void writeFixedSizeBytes(QueuedWriter writer, BankAddress instructionAddress,
        ...
        if (writer instanceof BpsWriter) {
            // We need to add a new hunk for the copy and then another new one for the next read
            // segment
            ...
            throw new IllegalArgumentException(
                    "SourceTargetCopyInstruct is only intended to be used with BpsWriter");
        }
    }
}
